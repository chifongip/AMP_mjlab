...
def _update_assets(assets: dict[str, bytes], source_dir: Path, _meshdir: str) -> None:
    """Populate assets dict with file contents from source_dir (replacement for removed mjlab update_assets)."""
    if not source_dir.exists():
        return
    for fpath in source_dir.rglob("*"):
        if fpath.is_file():
            assets[str(fpath.relative_to(source_dir))] = fpath.read_bytes()
...
def get_assets(meshdir: str) -> dict[str, bytes]:
  ...
  _update_assets(assets, G1_XML.parent / "assets", meshdir)
  return assets
